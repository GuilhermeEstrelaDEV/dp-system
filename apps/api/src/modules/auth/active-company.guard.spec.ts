...
describe('ActiveCompanyGuard', () => {
  const companyContext = Object.freeze({
    userId: 'user-1',
    companyId: '11111111-1111-4111-8111-111111111111',
    assignmentIds: Object.freeze(['assignment-1']),
  ...
  it('denies an authenticated request without an active company', async () => {
    const guard = new ActiveCompanyGuard({ resolve: jest.fn() } as never);
    await expect(
      guard.canActivate(
  ...
  it('delegates to the canonical resolver and attaches its immutable context', async () => {
    ...
    const guard = new ActiveCompanyGuard({ resolve } as unknown as ActiveCompanyResolverService);
    const request = {
      principal: {
        actorId: 'user-1',
