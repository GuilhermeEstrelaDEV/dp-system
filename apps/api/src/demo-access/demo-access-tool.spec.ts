...
describe('DemoAccessTool grants', () => {
  it('creates exactly thirty-five manual, temporary and approved RolePermission assignments', async () => {
    ...
    expect(deps.governance.createRolePermission).toHaveBeenCalledTimes(35);
  ...
  it('fails closed when the capability catalog is not exactly the approved 48 entries', async () => {
    ...
    await expect(new DemoAccessTool(deps).grant(validEnvironment)).rejects.toThrow(
      'catálogo esperado=48, encontrado=30',
    );
    expect(deps.governance.createRolePermission).not.toHaveBeenCalled();
  });
});
...
describe('DemoAccessTool status and revocation', () => {
  ...
  it('revokes only active approved-source capabilities and preserves rows through governance', async () => {
    ...
    expect(deps.governance.revokeRolePermission).toHaveBeenCalledTimes(35);
