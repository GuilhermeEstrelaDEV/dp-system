...
| BDP-014         | Homologar critérios, evidência canônica, autorização e efeitos da reabertura no fechamento de competência.        | ETP-014, integração operacional e auditoria.                  | Crítica    | DP, Financeiro, Segurança e TI      | APPROVED — VERSION 1 |
| BDP-AUTH-LEGACY | Homologar autorização, isolamento e transição das APIs legadas.                                                   | Matriz de capabilities, rollout, compatibilidade e auditoria. | Crítica    | Arquitetura e governança do projeto | APPROVED             |

## Resolução da BDP-AUTH-LEGACY

- DAL-01 a DAL-14 foram homologadas em 28/07/2026 pelo responsável pela arquitetura e governança;
- a [resolução v1](BDP-AUTH-LEGACY_RESOLUTION_V1.md) preserva decisões, justificativas, impactos,
  exceções e referências RACI;
- a aprovação conclui a governança documental e não inicia trabalho técnico nem cria assignments.
...
- BDP-014 foi homologada como `APPROVED — VERSION 1` na [resolução formal](BDP-014_RESOLUTION_V1.md). A [proposta anterior](BDP-014_RESOLUTION_PROPOSAL.md) permanece apenas como histórico das alternativas.
- O contrato canônico, a execução/review elegíveis, os blockers, a reabertura, a idempotência, as capabilities, a imutabilidade, o manifesto e o warning de remuneração variável são vinculantes para a ETP-014 v1.
