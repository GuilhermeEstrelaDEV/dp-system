...
| BDP-014 | Homologar critérios, evidência canônica, autorização e efeitos da reabertura no fechamento de competência.        | ETP-014, integração operacional e auditoria.              | Crítica    | DP, Financeiro, Segurança e TI   | PENDING APPROVAL |
...
- BDP-014 consolida as decisões D-014-01 a D-014-10 da [especificação](ETP-014_PAYROLL_PERIOD_CLOSURE_SPECIFICATION.md). A [proposta de resolução](BDP-014_RESOLUTION_PROPOSAL.md) apresenta alternativas e recomendações não vinculantes; nenhuma decisão está aprovada.
...
- Até a homologação explícita da proposta, BDP-014 permanece `PENDING APPROVAL` e a ETP-014 permanece em `PLANNING`, sem autorização para schema, migration, API ou frontend.
